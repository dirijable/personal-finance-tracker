...
public class TransactionService {
    ...
    @Transactional
    public void deleteById(Long transactionId, Long userId, Long accountId) {
        ...
        if(!account.getId().equals(accountId))
            throw new AccessDeniedException("account with id='%d' haven`t transaction with id='%d'".formatted(account.getId(), transaction.getId()));
    ...
    @Transactional
    public TransactionResponseDto create(TransactionCreateDto dto, Long userId, Long accountId) {
        ...
        if(!account.getUser().getId().equals(userId))
            throw new AccessDeniedException("User with id='%d' don`t have account with id='%d'".formatted(userId, accountId));
        ...
        if(dto.transactionType().equals(TransactionType.EXPENSE) && account.getBalance().compareTo(dto.amount()) <0)
            throw new NotEnoughMoneyException("account with id='%d' haven`t enough money".formatted(accountId));
